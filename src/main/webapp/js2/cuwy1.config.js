var config = {
	siteMap:{
		title:"ХОЛ::Призначеня",url:"home.html"
		,name:"Листи призначень"
		,subname:"Відділення реанімації"
		,subname2:"Відділення реанімації хмельницька обласна лікарня"
		,siteMaps:[
			{title:"Пацієнти",name:"Пацієнти",url:"patients.html"},
			{title:"ХОЛ::Ліки",name:"Ліки",url:"drugs.html"},
			{title:"Редактор: Лист призначень реанімація"
				,name:"Редактор: Лист призначень 24 години на добу - реанімація ", url:"lp24h.html"},
			{title:"ХОЛ::Протоколи",name:"Клінічні протоколи",url:"protocols.html"},
			{title:"IX::Пацієнт",name:"IX::Пацієнт",url:"patient.html"},
			{title:"Ліка додаток",name:"Ліка додаток",url:"drug.html"}
		]
	},
	startHour24lp: 8
};